...
Productquery = select {3}  as ProductName  from {0}master sm  join {0}Relation mr on mr.{0}ID = sm.{0}ID join CategoryMaster cm on cm.CategoryID = sm.CategoryID  Inner Join Targetmaster tm on {4} where TargetID = {1} and name = {2} order by sm.Sequence
